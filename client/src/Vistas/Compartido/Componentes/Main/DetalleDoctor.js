					...
					<section className="bg-white w-[550px] h-fit mt-10 flex flex-row p-3 rounded items-center">
						...
						<div className="ml-4">
							...
							<div class="relative bg-blue-800 flex justify-center">
								...
								<div class="absolute my-12 py-4">
									<a
										...
										target="_blank"
									>
										<span class=" z-1 text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
											Ampliar
										</span>
									</a>
								</div>
							...
							<div className="text-[#1479FF] mt-2 mb-2 ">
								&#9733;{" "}
								<span className="font-raleway text-[#292f53b8]">
									{doctor?.rating}
								</span>
								{/* {<StarDetail
              stars={4}/>} */}
					...
					<section className="bg-white w-[550px] h-fit mt-10 rounded-t">
						<h1 className="bg-[#1479FF] font-poppins text-white h-10 align-middle	p-2 rounded-t">
							Reseñas
						</h1>
						<p className="font-raleway text-[#030304b8] text-sm mt-4 mb-2 text-center">
							{doctor?.name} Aun no tiene reseñas
						</p>
						<br></br>
					</section>
				</div>
				...
				<div>
					<section className="bg-white w-[550px] h-fit mt-10 rounded mb-2">
						<p className="bg-[#1479FF] font-poppins text-white h-10 align-middle	p-2 rounded-t">
							Agenda tu cita
						</p>

						<p className="font-raleway text-[#292f53b8] text-sm mt-2 mb-2 ml-2">
							{" "}
							Direccion: {doctor && doctor.address}
						</p>

						<p className="font-poppins tracking-wide mt-1 mb-2 ml-2">
							{" "}
							Selecciona tu fecha:
						</p>
						...
						<div>
							{(() => {
								...
								for (let i = separateHours1A; i <= separateHours2A; i++) {
									td.push(
										<button
											className="font-raleway text-white mt-1 mb-2 focus:bg-[#292F53] rounded bg-[#1479FF] w-28 h-6 m-3"
											key={i}
											value={i + ":00"}
											onClick={(e) => handleClickHour(e)}
										>
											{i + ":00"}
										</button>
									);
						...
						{selectedDate.length > 1 ? (
							<p className="font-raleway text-[#292f53b8] text-sm mt-9 mb-2 ml-2">
								Tu cita esta agendada para el{" "}
								<span className="font-poppins tracking-wide mt-1 mb-2  text-center text-[#1479FF] ml-2">
									{selectedDate}{" "}
								</span>{" "}
								a las{" "}
								<span className="font-poppins tracking-wide mt-1 mb-2 ml-2 text-[#1479FF]">
									{selectedHour}{" "}
								</span>
							</p>
						) : (
									...
									to={
										user?.rol === "ADMIN"
											...
											: user?.rol === "DOCTOR"
												? "/doctor/doctors"
												: user?.rol === "PATIENT"
													? "/patient/buy/doctor/" + idDoctor
													: "/login"
									}
									className="font-poppins text-lg text-white text-center  focus:bg-[#292F53] rounded bg-[#1479FF] w-40 h-10 m-3 mt-8 pt-1"
								>
									{" "}
									Reserva tu cita
								</Link>
