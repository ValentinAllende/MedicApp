...
import { getPatientsBetweenDates } from '../../../../../Redux/actions/generalActionsPatients';
...
const BarChartPatients = () => {
  const { querySearch } = useSelector((state) => state.generalPatients);
  ...
  const data = {
    ...
    datasets:[
      {
        label: "Pacientes Registrados los últimos 5 días",
  ...
  useEffect(()=>{
    dispatch(getPatientsBetweenDates(startDate, finishDate));
  },[]);
